import sys
import importlib
import subprocess
import shutil
import os

def get_tool_version(command, version_flag='--version'):
    try:
        executable = command.split()[0]
        if executable != 'java' and shutil.which(executable) is None and not os.path.exists(executable):
             return "not_found"
        
        try:
            output = subprocess.check_output(f"{command} {version_flag}", shell=True, stderr=subprocess.STDOUT).decode().strip()
        except subprocess.CalledProcessError as e:
            output = e.output.decode().strip()
            
        lines = output.split('\n')
        
        if 'bwa' in command:
            for line in lines:
                if "Version:" in line:
                    return line.split('Version:')[1].strip()
        
        if 'miniasm' in command:
            return lines[0].strip()

        if 'medaka' in command:
            for line in lines:
                if "medaka" in line:
                    return line.strip()

        if 'kleborate' in command:
            for line in lines:
                if "Kleborate v" in line:
                    return line.strip()

        if 'flye' in command:
            for line in lines:
                if "Flye" in line:
                    return line.strip()

        if 'fastp' in command:
            for line in lines:
                if "fastp" in line and "." in line:
                    return line.split('fastp')[1].strip()

        for line in lines:
            if "Looking to launch" in line:
                continue
            if any(c.isdigit() for c in line) and '.' in line:
                return line.strip().replace('"', '')
                
        return lines[0].strip()
    except Exception as e:
        return f"error"

def main():
    packages = [
        "pandas", "numpy"
    ]

    print("python_tools:")
    print(f"  python: {sys.version.split()[0]}")
    
    for p in packages:
        try:
            mod = importlib.import_module(p)
            version = getattr(mod, "__version__", "unknown")
            print(f"  {p}: {version}")
        except ImportError:
            print(f"  {p}: not_installed")

    print("system_tools:")
    
    tools = {
        "fastp": ("fastp", "--version"),
        "fastqc": ("fastqc", "--version"),
        "chopper": ("chopper", "--version"),  
        "megahit": ("megahit", "--version"),
        "spades": ("spades.py", "--version"), 
        "flye": ("flye", "--version"),        
        "bwa": ("bwa", ""), 
        "samtools": ("samtools", "--version"),
        "minimap2": ("minimap2", "--version"),
        "miniasm": ("miniasm", "-V"),
        "medaka": ("medaka", "--version"),   
        "kleborate": ("kleborate", "--version"),
        "chewBBACA": ("chewBBACA.py", "--version"),
        "multiqc": ("multiqc", "--version"),
        "java": ("java", "-version"),
    }

    for name, (cmd, flag) in tools.items():
        ver = get_tool_version(cmd, flag)
        print(f"  {name}: \"{ver}\"")

if __name__ == "__main__":
    main()